...
class Client:
    def __init__(self, url, token):
        self.url = url
        self.token = token
        self.selected = post(url + "logs/", headers={'Authorization': token})
    ...
    def logs(self):
        ...
        selected = post(self.url + "logs/", headers={'Authorization': self.token})
        return [logs.Log(log) for log in loads(selected.text)["logs"]]

    def add_log(self, light: bool):
        """Add a log to the db."""
        selected = post(self.url + "logs/add/", data={'light': light}, headers={'Authorization': self.token})
        return [logs.Log(log) for log in loads(selected.text)["logs"]]
